...
public static class GeneralRegisterEndpoints
{
    public static void MapGeneralRegisterEndpoints(this WebApplication app)
    {
        ...
        group.MapPost("/", async ([FromForm] GeneralRegisterDto dto, IGeneralRegisterService service, IRecaptchaService recaptcha, IOutputCacheStore cache) =>
        {
            ...
            await cache.EvictByTagAsync("general-regs", default);
            return Results.Json(result.Data, statusCode: result.StatusCode);
        });

        group.MapPost("/save-photo-url", async ([FromForm] GeneralDocumentDto dto, IGeneralRegisterService service, IOutputCacheStore cache) =>
        {
        ...
        group.MapPut("/{id:int}/paymentstatus", async (int id, PaymentStatusUpdateDto dto, IGeneralRegisterService service, IOutputCacheStore cache) =>
        {
